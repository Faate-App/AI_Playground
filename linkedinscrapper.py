...
import time
import asyncio
async def scrapper(prenom, nom):
    dataperson = {"prenom": prenom, "nom": nom, "localisation": "", "titre": "", "education": "","link": ""}
    async with sync_playwright() as p:
        try:
            ...
            try:
                    ...
                    page.wait_for_selector(profil_link_selector)
                    ...
                    try:
                        page.wait_for_selector("span.text-body-small.inline.t-black--light.break-words")
                        localisation = page.inner_text("span.text-body-small.inline.t-black--light.break-words")
                        dataperson["localisation"] = localisation
                        print(f"Localisation: {localisation}")
                    except:
                        print("Problem with "+str(prenom)+" "+str(nom))
                    try:
                        page.wait_for_selector("div.JLTOxdAyPIbwAVfujRhErCSfXZcrNbXpHcBIE")
                        titre = page.inner_text("div.JLTOxdAyPIbwAVfujRhErCSfXZcrNbXpHcBIE")
                        dataperson["titre"] = titre
                        print(f"Titre: {titre}")
                    except:
                        print("Problem with "+str(prenom)+" "+str(nom))
                    try:
                        page.wait_for_selector("section.pvs-list")
                        education = page.inner_text("section.pvs-list")   
                        dataperson["education"] = education
                        print(education)
